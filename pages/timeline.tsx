	...
	render(): React.ReactNode {
		return (
			<button
				...
				className={`py-1 px-1 text-xs font-medium text-gray-900 bg-white rounded-l-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 ${
					this.state.isActive
...
class EventRecord extends React.Component<EventRecordProps> {
	...
	render(): React.ReactNode {
		return (
			isInActivate(this.props.event.people, this.props.activePeople) && (
				<li className="mb-10 ml-4" key={this.props.index}>
					<div className="absolute w-3 h-3 bg-gray-200 rounded-full mt-1.5 -left-1.5 border border-white dark:border-gray-900 dark:bg-gray-700"></div>
					<div className="flex justify-between mx-auto">
						<time className="mb-1 text-sm font-bold leading-none text-black dark:text-gray-300">
							{this.props.date}
						</time>
						<div className="flex mb-2 text-sm font-normal leading-none text-black space-x-1">
							{this.props.event.people.map((p) => {
								return (
									<div className=" text-[#D48166]">{p}</div>
								);
							})}
						</div>
					</div>
...
> {
	...
	register = (name: string, toggle: boolean) => {
		this.setState({
			map: new Map<string, boolean>(this.state.map.set(name, toggle)),
		});
	};
	...
	render(): React.ReactNode {
		...
		return (
			<div className="flex flex-col">
				<p className="text-sm font-noto text-gray-400">
					按事件人物筛选:
				</p>
				...
				{data.map((timeline, index) => {
					return (
						<button
							...
							type="button"
							className="py-1 px-1 text-xs font-medium text-gray-900 bg-white rounded-l-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-blue-500 dark:focus:text-white"
						>
							{timeline.category}
						</button>
					);
			...
			{data.map((timeline, index) => {
				return (
					timeline.category == category && (
						<CategoryGroup timeline={timeline} key={index} />
					)
				);
			})}
		</div>
