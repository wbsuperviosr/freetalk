...
type DropdownProps = {
	children?:
		| React.ReactElement<DropdownItem>
		| React.ReactElement<DropdownItem>[];
...
export class DropdownItem extends React.Component<DropdownItemProps> {
	render(): React.ReactNode {
		return (
			...
			<a
				href="#"
				className="flex h-[50px] items-center p-3 rounded-lg transition-[background] duration-500 hover:bg-slate-100"
				onClick={() =>
					this.props.goToMenu &&
					this.props.setActiveMenu(this.props.goToMenu)
				}
			>
				<span className="icon-button mr-1">{this.props.leftIcon}</span>
				{this.props.children}
				<span className="icon-right ml-auto">
...
> {
	...
	componentDidMount(): void {
		this.setState({
			height: this.dropdownRef.current?.firstChild.offsetHeight,
		});
	}
	...
	render(): React.ReactNode {
		return (
			<div
				className={`absolute h-${this.state.height} shadow-md top-[260px] w-[200px] overflow-hidden translate-x-[-45%]  transition-height duration-300 ease-in-out`}
	...
	render(): React.ReactNode {
		return (
			<li className="flex justify-center items-center w-14">
				<a
					href="#"
					className="flex items-center justify-center transition-[filter] duration-300 hover:brightness-200"
		...
		return (
			<header className="flex justify-between p-5">
				...
				<div>
					<Dropdown
						icon={
							<Bars3Icon className="h-8 w-8 bg-black text-white rounded-full p-[1.5px] [&>path]:stroke-[3px]" />
						}
					>
